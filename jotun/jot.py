...
async def scrape_jotun_jobs():
    base_url = "https://jotun.wd3.myworkdayjobs.com"
    jobs_url = base_url + "/Jotun_careers?locationCountry=9e684fd7be1e469d9ee955a4c3b754be"
    ...
    async with async_playwright() as p:
        ...
        # if not found print no jobs available
        if not job_cards:
    ...
    with open("jotun_jobs.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=keys)
        writer.writeheader()
        writer.writerows(results)

    print(f"Saved {len(results)} jobs to jotun_jobs.csv")
    return results

if __name__ == "__main__":
    asyncio.run(scrape_jotun_jobs())
