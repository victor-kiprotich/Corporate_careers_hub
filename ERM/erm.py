...
async def scrape_erm_jobs():
    base_url = "https://erm.wd3.myworkdayjobs.com"
    jobs_url = f"{base_url}/ERM_Careers?locationCountry=9e684fd7be1e469d9ee955a4c3b754be"
    ...
    async with async_playwright() as p:
        ...
        for i, card in enumerate(job_cards):
            ...
            preview_title = await title_el.inner_text() if title_el else ""
            ...
            try:
                ...
                job_title = await job_title_el.inner_text() if job_title_el else preview_title
            except:
    ...
    with open("erm_jobs.csv", "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=keys)
        writer.writeheader()
        writer.writerows(results)

    print(f"Saved {len(results)} jobs to erm_jobs.csv")
    return results

if __name__ == "__main__":
    asyncio.run(scrape_erm_jobs())
